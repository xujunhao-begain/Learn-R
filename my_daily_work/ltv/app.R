...
#     # $l1_user_group
#     # selectInput(
#     #   inputId = "l1_user_group_",
#     #   label = mapping_colname(map_ls, "l1_user_group"),
#     #   choices = c("思维","NA","科特"),
#     #   selected = c("思维","NA","科特"),
#     #   multiple = TRUE)
#     
...
#         
#         print(0)
#         print(input[[paste0(name, "_")]])
# 
#         if(!is.null(input[[paste0(name, "_")]])) {
# 
...
server <- function(input, output) {
  ...
  dat <- eventReactive(input$update,{
    ...
    flt_sele_after <-
      ...
      group_by(across(all_of(as.character(input$dims))),datedif) %>%
      summarise(total_gmv = sum(total_gmv),
                #user_cnt = sum(ifelse(datedif == 0 & go_course_type_detail == '编程L1-首单',user_cnt, 0))
                user_cnt = sum(ifelse(datedif == 0 & go_course_type_detail %in% input$course,user_cnt, 0))
                #.groups = "drop"
      ) %>%
      #view()

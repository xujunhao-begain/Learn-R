...
# 1n
#  y  n y  i1
# 更一般地，用最小二乘法估计多元线性回归、非线性回归(拟合)的待定参数，也是类似 的，只需要将线性预测值改成模型预测值即可，具体公式如下:
#   n
# argminJβ [y fx,β]2
# β
...
#图(e)和图(f)属于模型本身有问题，没有把模型部分提取完全;图(b)说明数据有异常点， 应处理掉异常点重新建模;图(c)中残差随 x 的增大而增大;图(d)中残差随 x 的增大而先增 后减，两者都属于异方差。此时应该考虑在回归之前对数据 y 或 x 进行变换，实现方差稳定后再 建模。原则上，当残差方差变化不太快时采用开根号变换 y ;当残差方差变化较快时取对数变
# 换 ln y;当残差方差变化很快时取逆变换 1/y;还有其他变换，如著名的 Box-Cox 变换或
# Yeo-Johnson 变换(可应付负值)，可将非正态分布数据变换为正态分布。 因此，用残差检验模型是否成功，就是对残差做正态性检验。也可以进一步考察学生化残
...
#显著。另外，构建自然样条特征也是引入非线性关系的更好的做法。
                                                                                                                           年到老年)的差异，很可能会显著。另外，构建自然样条特征也是引入非线性关系的更好的做法。 将三个数值变量的二次项以及交互项 sex:island 加入模型，再逐步回归剔除不显著项，
#代码如下:

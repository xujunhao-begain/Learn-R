...
4.访问向量子集
访问向量子集即访问向量的一些特定元素或者某个子集。注意，R 中的索引是从 1 开始的。
使用元素的位置来访问，形式如下所示:
  v1 = c(1, 2, 3, 4)
...
v1[3:6]
使用逻辑向量来访问，输入与向量相同长度的逻辑向量，以此决定每一个元素是否要被 获取:
  v1[c(TRUE, FALSE, TRUE, FALSE)]
这可以引申为“根据条件访问向量子集”:
  5.为向量子集赋值，替换相应元素 为向量子集赋值，就是先访问到向量子集，再赋值。
v1[2] = 0
注意，若对不存在的位置赋值，前面将用 NA 补齐: v1[10] = 8
v1
6.为向量元素命名 你可以在创建向量的同时对其每个元素命名，代码如下:
  x = c(a = 1, b = 2, c = 3)
x
## a b c ## 1 2 3
...
函数 order()可以返回元素排好序的索引，以其结果作为索引访问元素，正好是排好序的 向量。

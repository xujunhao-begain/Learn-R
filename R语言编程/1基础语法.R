...
## [1] "a" "b" "c"
也可以创建列表后再对列表成分命名或修改名字:
  2.从列表中提取成分的内容 提取列表中某成分下的内容，最常用的方法是用$，通过成分名字来提取该成分下的内容:
  l1$y
l1$m # 访问不存在的成分m, 将会返回NULL
也可以用[[n]]来提取列表第 n 个成分的内容，n 也可以换成成分的名字:
  l1[[2]] # 同l1[["y"]] 用[[]]提取列表中某个成分的内容则更加灵活，可用在函数调用中，通过参数来传递成分
的名字或索引:
  3.提取列表子集
...
用[]提取若干成分时，返回列表的子集，还是一个列表;用[[ ]]提取单个成分的元素，返回的是对应 成分的元素。 总之，[]用于提取对象的子集，类型仍是该对象;[[]]用于提取对象的内容(即下一级元素)。
4.为列表的成分赋值
即先访问(提取)到列表的成分，再赋以相应的值。注意，若给一个不存在的成分赋值， 列表会自动地在对应名称或位置下增加一个新成分。
names(l1) = NULL # 移除列表成分的名字 names(l1) = c("x","y","z")
p = "y" #想要提取其内容的成分名字 l1[[p]]
l1["x"] # 同l1[1]
l1[c("x", "z")] # 同 l1[c(1, 3)], l1[c(TRUE, FALSE, TRUE)]
异步社区cloudcHCO8tlw0iz(15725519632) 专享 请尊重版权

1.3 数据结构:列表、数据框、因子 17
l1$x = 0 # 将列表的成分x赋值为0 也可以同时给多个列表成分赋值:
l1[c("x", "y")] = list(x = "new value for y", y = c(3, 1))
若要移除列表中的某些成分，只需赋值为 NULL: l1[c("z", "m")] = NULL
5.列表函数
...
## $b
## [1] 2
通过去列表化函数 unlist()可将一个列表打破成分界线，强制转换成一个向量1: unlist(l2)
## a b ## 1 2
为了方便操作列表，tidyverse 系列中的 purrr 包提供了一系列列表相关的函数，建议读者查阅并 使用。
 pluck():同[[]]提取列表中的元素。
 keep():保留满足条件的元素。
 discard():删除满足条件的元素。
 compact():删除列表中的空元素。
...
 flatten():摊平列表(只摊平一层)。
1.3.2 数据框(数据表)
R 语言中用于统计分析的样本数据，都是按数据框类型操作的。
数据框是指有若干行和列的数据集，它与矩阵类似，但并不要求所有列都是相同的类型。 本质上讲，数据框就是一个列表，它的每个成分都是一个向量，并且长度相同，以表格的形式 展现。总之，数据框是由列向量组成、有着矩阵形式的列表。
数据框与常见的数据表是一致的:每一列代表一个变量属性，每一行代表一条样本数据。 以表 1.1 所示的数据表为例。
表 1.1
数据表示例
24 25 23
Name
...
Age
Major
Ken Male Ashley Female Jennifer Female
Finance Statistics Computer Science
1 若列表的成分具有不同类型，则自动向下兼容到同一类型。
异步社区cloudcHCO8tlw0iz(15725519632) 专享 请尊重版权
...
 tibble()比 data.frame()做的更少:不改变输入变量的类型(注:R 4.0.0 之前默
                                        认将字符串转化为因子)，不会改变变量名，不会创建行名。
...
用 tibble()根据若干列向量创建 tibble:
  ## # A tibble: 3 x 4
  library(tidyverse) # 或tibble persons = tibble(
...
4c
NA d
1 读者若习惯用 R 自带的 data.frame，只需要换个名字，将 tibble 改为 data.frame 即可。 异步社区cloudcHCO8tlw0iz(15725519632) 专享 请尊重版权
...
例如，可以用 $ 按列名来提取某一列的值，或者用[[]]按照位置或列名提取。 例如，提取列名为 x 的值，并得到向量:
  df$x # 同df[["x"]], df[[2]] ##[1] 0 2 1-1
以列表形式构建子集完全适用于数据框，同时也会生成一个新的数据框。提取子集的操作 符[]允许用数值向量表示列的位置，用字符向量表示列名，或用逻辑向量指定是否选择。
例如，提取数据框的一列或多列，可以得到子数据框:
  ## ## ##1 ##2 ##3 ##4
  id <int> 1 2 3 4
1.3 数据结构:列表、数据框、因子 19
## ##1 ##2 ##3 ##4
<int> <dbl> <dbl> 1 00.5 2 20.2 3 10.1 4 -1 0.5
df[1]
## # A tibble: 4 x 1
# 提取第1列, 同df["id"]
# 同df[c("id","x")], df[c(TRUE,TRUE,FALSE)]
df[1:2]
## # A tibble: 4 x 2
##      id     x
## ##1 ##2 ##3 ##4
...
3     1
4    -1
(2)以矩阵方式提取数据框的元素、子集 以列表形式操作并不支持行选择，以矩阵形式操作则更加灵活。若将数据框看作矩阵，其
二维形式的存取器可以很容易地获取一个子集的元素，同时支持列选择和行选择。 换句话说，可以使用[i, j]指定行或列来提取数据框子集，[,]内可以是数值向量、字符
向量或者逻辑向量。
若行选择器为空，则只选择列(所有行):
  df[, "x"]
## # A tibble: 4 x 1
...
##       x     y
## ##1 ##2 ##3 ##4
<dbl> <dbl> 00.5 20.2 10.1 -1 0.5
若列选择器为空，则只选择行(所有列):
  df[c(1,3),]
## # A tibble: 2 x 3
##      id     x     y
## ##1 ##2
<int> <dbl> <dbl> 1 00.5 3 10.1
同时选择行和列:
  df[1:3, c("id","y")]
## # A tibble: 3 x 2
##      id     y
## ##1 ##2 ##3
<int> <dbl> 10.5 20.2 30.1
根据条件筛选数据。例如用y >= 0.5筛选df的行，并选择id和y两列: df[df$y >= 0.5, c("id","y")]
## # A tibble: 2 x 2
##      id     y
## ##1 ##2
<int> <dbl> 10.5 40.5
按列名属于集合{x, y, w}来筛选df的列，并选择前两行: ind = names(df) %in% c("x","y","w")
df[1:2, ind]
## # A tibble: 2 x 2
##       x     y
## ##1 ##2
<dbl> <dbl> 00.5 20.2
3.给数据框赋值
给数据框赋值就是选择要赋值的位置，再准备好同样大小且格式匹配的数据，赋值给那些 位置即可，同样有列表方式和矩阵方式。
(1)以列表方式给数据框赋值
用 $ 或[[ ]]对数据框的某列赋值
...
## ##1 ##2 ##3 ##4
<int> <dbl> <dbl> <dbl> 1 00.50.5 2 20.22.2 3 10.11.1 4-10.5-0.5
...
<int> <dbl> <dbl> <chr>
  1 0 2 2 3 1 4 -1
0.5 0.5 0.2 2.2 0.1 1.1 0.5-0.5
用[]可以对数据框的一列或多列进行赋值: df["y"] = c(0.8,0.5,0.2,0.4)
df[c("x", "y")] = list(c(1,2,1,0), c(0.1,0.2,0.3,0.4))
(2)以矩阵方式给数据框赋值 以列表方式对数据框进行赋值时，也是只能访问列。若需要更加灵活地进行赋值操作，可
以通过矩阵方式进行。
df[1:3,"y"] = c(-1,0,1)
...
把 summary()作用在数据框或列表上，将生成各列或各成分的汇总信息: summary(persons)
1.3 数据结构:列表、数据框、因子 21
##      Name
##  Length:3
##  Class :character
Gender
...
##
##
我们经常需要将多个数据框(或矩阵)按行或按列进行合并。用函数 rbind()增加行(样 本数据)，要求宽度(列数)相同;用 cbind()函数增加列(属性变量)，要求高度(行数) 相同。
例如，向 persons 数据框中添加一个新记录: rbind(persons,
                                ## # A tibble: 4 x 4
                                ##   Name     Gender   Age Major
                                ##   <chr>    <chr>  <dbl> <chr>
...
##1 KenMale24 Finance ## 2 Ashley Female 25 Statistics ## 3 Jennifer Female 23 Computer Science
TRUE 3
TRUE 2 FALSE 3
rbind()和 cbind()不会修改原始数据，而是生成一个添加了行或列的新数据框。
函数 expand.grid()可生成多个属性水平值的所有组合(笛卡儿积)形式的数据框: expand.grid(type = c("A","B"), class = c("M","L","XL"))
## ##1 ##2 ##3 ##4 ##5 ##6
type class
A     M
B     M
...
B    XL
1.3.3 因子
数据(变量)可划分为:定量数据(数值型)、定性数据(分类型)，定性数据又分为名义 型(无好坏顺序之分，如性别)、有序型(有好坏顺序之分，如疗效)。
R 提供了因子(factor)这一数据结构(容器)，专门用来存放名义型和有序型的分类变量。 因子本质上是一个带有水平(level)属性的整数向量，其中“水平”是指事前确定可能取值的有 限集合。例如，性别有两个水平属性:男、女。
直接用字符向量也可以表示分类变量，但它只有字母顺序，不能规定想要的顺序，也不能 表达有序分类变量。所以，有必要把字符型的分类变量转化为因子型，这更便于对其做后续描 述汇总、可视化、建模等。
1.创建与使用因子
函数 factor()用来创建因子，基本格式为:
  factor(x, levels, labels, ordered, ...)
x:为创建因子的数据向量。
levels:指定因子的各水平值，默认为 x 中不重复的所有值。 labels:设置各水平名称(前缀)，与水平名称一一对应。 ordered:设置是否对因子水平排序，默认 FALSE 为无序因子，TRUE 为有序因子。 该函数还包含参数 exclude:指定有哪些水平是不需要的(设为 NA);nmax 用于设定水平数的上限。 若不指定参数 levels，则因子水平默认按字母顺序。
比如，现有 6 个人的按等级划分的成绩数据，先以字符向量创建，并对其排序: ## [1] "优" "中" "良" "优" "良" "良"
  sort(x) # 排序是按字母顺序 异步社区cloudcHCO8tlw0iz(15725519632) 专享 请尊重版权
x = c("优", "中", "良", "优", "良", "良") # 字符向量 x

## [1] "良" "良" "良" "优" "优" "中"
它的顺序只能是字母顺序，如果想规定顺序:中、良、优，正确的做法就是创建因子，用 levels 指定想要的顺序:
  ## [1]优中良优良良 ## Levels: 中 良 优
  as.numeric(x1) # x的存储形式: 整数向量 ## [1] 3 1 2 3 2 2
注意，不能直接将因子数据当字符型操作，需要用 as.character()转化。
转化为因子型后，数据向量显示出来(外在表现)与原来是一样的，但这些数据的内在存 储已经变了。因子的内在存储与外在表现如图 1.8 所示。因子型是以整数向量存储的，将各水平 值按照规定的顺序分别对应到整数，将原向量的各个值分别用相应的整数存储，输出和使用的 时候再换回对应的水平值。整数是有顺序的，这样就相当于在不改变原数据的前提下规定了顺 序，同时也节省了存储空间。
...
很多人将因子固有的顺序与有序因子混淆，二者不是一回事:上述反复提到的顺序，可称 为因子固有的顺序，正是有了它，才能方便地按想要的顺序进行排序、统计频数、绘图等;而 无序因子与有序因子，是与变量本身的数据类型相对应的，名义型(无顺序好坏之分的分类变 量)用无序因子存放，有序型(有顺序好坏之分的分类变量)用有序因子存放，该区分是用于 不同类型的数据，建模时适用不同的模型。

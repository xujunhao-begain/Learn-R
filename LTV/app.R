...
#     # calculate
#     # flt %>%
#     # group_by(pay_month, across(all_of(input$dims))) %>%
#     # summarise(!!sym(input$denominator) := sum(!!sym(input$denominator)),
#     #           !!sym(input$numerator) := sum(!!sym(input$numerator)),
#     #           .groups = "drop") %>%
#     # group_by(across(all_of(input$dims))) %>%
#     # mutate(rate =
#     #          roll_mean(!!sym(input$numerator), width = input$roll_n) /
#     #          roll_mean(!!sym(input$denominator), width = input$roll_n)) %>%
...
#     #   ) %>%
#     #   ggplot(aes(x = datedif, y = avg_gmv)) +
#     #   geom_line(aes(colour = across(all_of(input$dims)) , group =  across(all_of(input$dims))))
#     
#     print(3)
#     print(dat)
#     #class(dat())
...
#     plt <-
#       dat %>%
#       # #group_by(across(all_of(as.character(input$dims))),as.factor(datedif)) %>%
#       # #group_by(l1_user_group,datedif) %>%
#       # summarise(sum_gmv=sum(total_gmv),
#       #           sum_user_cnt = sum(user_cnt),
#       #           .groups = "drop") %>%
...
#     
#     #ggplot(aes(x = datedif, y = avg_gmv)) +
#     #geom_line(aes(colour = !!sym(input$dims), group =  !!sym(input$dims)))
#   })
#   
#   
#   output$linePlot <- plotly::renderPlotly({

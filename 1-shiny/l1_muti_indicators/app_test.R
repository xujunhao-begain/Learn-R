...
server <- function(input, output, id = "ads_l1_eda_multidimensional_indicators_df") {
  ...
  observeEvent(input$toggle4, {
    toggle("collapse4")  # 切换内容的可见性
    toggleClass("toggle4", "collapsed")  # 切换箭头方向
  })
  
  observeEvent(input$toggle5, {
    toggle("collapse5")  # 切换内容的可见性
    toggleClass("toggle5", "collapsed")  # 切换箭头方向
  })
  ...
  dat <- eventReactive(input$update, {
    ...
    if(length(par_expr) != 0) {
      ...
      scope <- scope  # 无筛选条件时保持原始数据
    }
    
    # print(4)
    # print(input$dims)
    # print(colnames(scope))
    ...
    # 将列名替换为用户选择的指标名称
    new_column_names <- c(names(scope_rate)[1:length(input$dims)], metric_names)  # 保留分组列

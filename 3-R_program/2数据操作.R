...
#另外，extract()函数可以利用正则表达式的分组捕获功能，直接从一列中提取出多组信 息，
并生成多个列。例如，处理本节开始的不整洁数据，代码如下所示:
...
#生成一个表，以匹配人物角色和他们的昵称，name 直接选择列，昵称来自列表列 titles, 
#纵向展开它:
chars1 %>%
select(name, title = titles) %>%
unnest_longer(title)

#或者改用 hoist()函数直接从内层提取想要的列，再对列表列 title 做纵向展开:
chars %>%
   hoist(char, name = "name", title = "titles") %>%
   unnest_longer(title)

#另外，还有 tibblify 包专门用于将嵌套列表转化为 tibble 数据框。 

#2.5 基本数据操作
# 用 dplyr 包实现各种数据操作，通常的数据操作无论多么复杂，往往都可以分解为若干基 
# 本数据操作步骤的组合。
# 共有 5 种基本数据操作:
# select()— 选择列;
# filter()/slice()— 筛选行;
...
# mutate()— 修改列/创建新列;
# summarize()— 汇总。
# 这些函数都可以与 group_by()— 分组函数连用，以改变数据操作的作用域(作用在整个数据
# 框，还是分别作用在数据框的每个分组)
# 这些函数组合使用就足以完成各种数据操作，它们的相同之处是:
# 第 1 个参数是数据框，方便管道操作;
# 根据列名访问数据框的列，且列名不用加引号; 
# 返回结果是一个新数据框，不改变原数据框。
# 通过把函数组合使用，可以方便地实现“将多个简单操作，依次用管道连接，并实现复杂的
# 数据操作”
# 另外，若要同时对所选择的多列数据应用函数，还可以使用强大的 across()函数，它支持
# 各种选择列语法，搭配 mutate()和 summarise()函数使用，能同时修改或汇总多列数据， 
# 非常高效。类似地，dplyr 包提供了 if_any()和 if_all()函数，搭配 filter()函数使用 
# 可以达到根据多列的值筛选行的目的。

#2.5.1 选择列
#选择列包括对数据框做选择列、调整列序和重命名列。 
...
# 选择指定列
# everything():选择所有列。
# last_col():选择最后一列，可以带参数，例如last_col(5)选择倒数第6列。 
# 选择列名匹配的列
# starts_with():得到以某前缀开头的列名。 
# ends_with():得到以某后缀结尾的列名。
# contains():得到包含某字符串的列名。
# matches():匹配正则表达式的列名。
# num_range():匹配数值范围的列名，如num_range("x",1:3)匹配x1、x2和x3。 
# 结合函数选择列
...
#也可以自定义返回 TRUE 或 FALSE 的判断函数，支持 purrr 风格的公式写法。
#例如，选 择列值之和大于 3000 的列:
df[, 4:8] %>%
select(where(~ sum(.x, na.rm = TRUE) > 3000))
#再比如，结合 n_distinct()选择唯一值数目小于 10 的列: 
df %>%
...
#注意:-ends_with()要放在 everything()后面，否则删除的列又全回来了。
...
#用 relocate()函数将选择的列移到某列之前或之后，基本语法为: 
...
#还有更强大的rename_with(.data, .fn, .cols)函数，参数“.cols”支持用选择列语法选择
#要重命名的列，“.fn”是对所选列重命名的函数，将原列名的字符向量变成新列名的字符向
#量。比如，将包含“m”的列名，都拼接上前缀“new_”:
df %>%
  rename_with(~ paste0("new_", .x), matches("m"))

#6.强大的 across()函数
# 函数 across()恰如其名，让零个/一个/多个函数穿过所选择的列，即同时对所选择的多列
# 应用若干函数，基本格式如下:
across(.cols = everything(), .fns = NULL, ..., .names)
#.cols 为根据选择列语法选定的列范围。
# .fns 为应用到选定列上的函数，它可以是以下类型。
#   NULL:不对列作变换。
#   一个函数，如 mean。 
#   一个purrr风格的匿名函数，如~ .X * 10。 
#   多个函数或匿名函数构成的列表。
# .names 用来设置输出列的列名样式，默认为{col}_{fn}。若想保留旧列，则需要设置该参
# 数，否则将使用原列名，即计算的新列将替换旧列。
# across()支持各种选择列语法，与 mutate()和 summarise()连用，可以同时修改/(多 种)
# 汇总多列效果。
# across()也能与 group_by()、count()和 distinct()函数连用，此时“.fns”为 NULL，只起
# 到选择列的作用。
#   across()函数的引入，使我们可以弃用那些限定列范围的后缀:_all、_if、_at。 
#   across(everything(), .fns):在所有列范围内，代替后缀_all。
#   across(where(), .fns):在满足条件的列范围内，代替后缀_if。
#   across(.cols, .fns):在给定的列范围内，代替后缀_at。
# across 函数的作用机制如图 2.20 所示，它包含了分解思维，即想要同时修改多列，只需 
# 要选出多列，并把对一列数据做的操作写成函数，剩下的交给 across()就行了。


#2.5.2 修改列
#修改列即修改数据框的列，并计算新列。
#1.创建新列
#用 dplyr 包中的 mutate()函数创建或修改列，返回原数据框并增加新列，默认加在最后 
#一列，参数.before 和.after 可以设置新列的位置。若改用 transmute()函数则只返回增 
#加的新列。

#若只给新列 1 个值，则循环使用并得到值相同的一列: 
df %>%
  mutate(new_col = 5)
...
# 在同一个 mutate()函数中可以同时创建或计算多个列，它们是从前往后依次计算，所以
# 可以使用前面新创建的列，例如:
# 计算 df 中 math 列的中位数;
...
#3.修改多列
# 结合 across()函数和选择列语法可以把函数应用到多列，从而实现同时修改多列。 
# (1)把函数应用到所有列
# 将所有列转化为字符型:
df %>%
...
#4.替换 NA
#(1)replace_na()函数
# 实现用某个值替换一列中的所有 NA 值，该函数接受一个命名列表，
# 其成分为“列名 = 替换值”。
# 替换具体的列的缺失值，代码如下:
starwars %>%
...
# tidyr 包中的 fill()函数适合处理这种结构的缺失值, 默认是向下填充，即用上一个非缺
# 失值填充:
gap_data %>%
  fill(site, species)
...
#用 case_when()函数做多分支判断进而重新编码，避免使用很多 if_else()嵌套: 
df %>%
...
# 组变量、设置与替换缺失值等。sjmisc 包也支持标签化数据，这对操作 SPSS 或 Stata 
# 数据集特别有用。
...
# 重编码对:每个重编码对用“;”隔开，例如rec="1=1; 2=4; 3=2; 4=3"。 
# 多值:把多个旧值(逗号分隔)重编码为一个新值，例如 rec="1,2=1; 3,4=2"。 
# 值范围:用冒号表示值范围，例如rec="1:4=1; 5:8=2"。
# 数值型值范围:带小数部分的数值向量，值范围内的所有值将被重新编码，
#   例 如rec="1:2.5=1; 2.6:3=2"1
# min 和 max:最小值和最大值分别用 min 和 max 表示，
#   例如 rec = "min:4=1; 5:max=2"(min和max也可以作为新值，如5:7=max, 表示将5~7编码为max(x))。 
# else:所有未设定的其他值都用 else 表示，例如 rec="3=1; 1=2; else=3"。 
# copy:else可以结合copy一起使用，表示所有未设定的其他值保持原样(从 原数值copy)，
# 例如rec="3=1; 1=2; else=copy"。
# NAs:NA既可以作为旧值，也可以作为新值，例如rec="NA=1; 3:5=NA"。 
# rev:设置反转值顺序。
...
#注意:新值的值标签可以在重新编码时一起设置，只需要在每个重编码对后接上中括号标签。
...
#提供筛选条件给 filter()函数则返回满足该条件的行。
#筛选条件本质上是用长度同行数 的逻辑向量，通常是直接用能返回这种逻辑向量的列表达式。
df_dup %>%
  filter(sex == "男", math > 80)
#注意:对多个条件之间用“,”隔开，相当于 and。 
df_dup %>%
...
# if_all()是对这些逻辑向量依次取&，if_any()是对这些逻辑向量依次取|，最终得到一个
# 逻辑向量并将其用于filter()筛选行。
# 注意:对多个逻辑向量做&或|时，是做向量化运算，相当于是对位于同行的逻辑值取&或|，
# 换句话说，相当于将函数.fns 依次作用在所选列的每一行元素上，得到的判断结果，
# 取&或|，再作为是否筛选该行的依据。
...
#另一种思路:pmap_lgl()是对数据框逐行迭代，返回长度同行数的逻辑值向量，正好适 
#合配合 filter()筛选行。filter()函数的第一个参数是由多列范围构成的数据框;
#第2个参数是对多列范围内的每行的值向量构造一个可返回逻辑值的判断函数，并将该
#逻辑值作为是否筛选该行的依据。
#例如，筛选出语文、数学、英语三科成绩中恰有两科成绩不及格(分数 < 60)的行: 
df %>%
...
df %>% #选择 math 列值排在前 5 的行: 
  slice_max(math, n = 5)

#4.删除行 
#(1)删除重复行
#用 dplyr 包中的 distinct()函数删除重复行(只保留第 1 个，删除其余)
df_dup %>%
  distinct()
...
df_dup %>%
  drop_na(sex:math)
#若要删除某些列都是 NA 的行，借助 if_all()函数也很容易实现: 
df_dup %>%
...
#若要按递减进行排序，嵌套一个 desc()函数或在变量名前加负号即可: 
df_dup %>%
  arrange(-math) # 同desc(math), 递减排序

#2.5.5 分组操作
# 对未分组的数据框，一些操作(如 mutate()函数)是在所有行上执行。相当于把整个数据框
# 视为一个分组，所有行都属于它。
# 若数据框被分组，则这些操作是分别在每个分组上独立执行。可以认为是，将数据框拆分
# 为更小的多个数据框。在每个更小的数据框上执行操作，最后再将结果合并回来。
# 1.创建分组
# 用 group_by()函数创建分组，只是对数据框增加了分组信息(可以用 group_keys() 查看)，
# 并不是真的将数据分割为多个数据框。
df_grp = df %>%
  group_by(sex)
...
# purrr 风格的分组迭代:将函数.f 依次应用到分组数据框(即.data)的每个分组上。 
...
#2.分组修改
# 分组是一种强大的数据思维，当你想分组并分别对每组数据进行操作时，
# 应该优先采用 “group_by + mutate”，而不是“分割数据 + 循环迭代”。
# 这里仍是数据分解的思维:一旦要对数据框分组，你只需要考虑对一个分组(子数据框)做
# 的操作怎么实现，剩下的事情(如“分组 + 合并结果”)，“group_by + mutate”会帮你完成。

# 例如，对如下的股票数据，分别计算每只股票的收盘价与前一天的差价。
load("data/stocks.rda")
stocks
#只要对 Stock 进行分组，对一只股票怎么计算收盘价与前一天的差价，
#就可以按相同的方式编写代码:
stocks %>%
  group_by(Stock) %>%
...
#3.分组筛选
# filter()是根据条件筛选数据框的行，与 group_by()连用，就是分别对每个分组，
# 根据条件筛选行，再将结果合并到一起返回。
# 这里仍是数据分解的思维:一旦对数据框分组，你只需要考虑对一个分组(子数据框) 如何
# 构造条件筛选行，至于剩下的事情—“分组+合并结果”，“group_by + filter” 会帮你完成。
...
#比较推荐的写法是先用 mutate 计算出新列(涨幅列)，再构造筛选条件: s
tocks %>%
  group_by(Stock) %>%
  mutate(Gains = (Close - lag(Close)) / lag(Close)) %>%
  filter(Gains > 0.04)
#另外，group_by 也可以与 slice_*连用，按分组切片的方式筛选行。
#例如，筛选每 只股票的收盘价位于从高到低排序的前两名的收盘价:
stocks %>%
  group_by(Stock) %>%
  slice_max(Close, n = 2)

#4.分组汇总
# 分组汇总，相当于 Excel 的透视表功能。 对数据框做分组最主要的目的就是做分组汇总，
# 汇总就是以某种方式组合行，可以用 dplyr包中的 summarise()函数实现，结果只保留
# 分组列的唯一值和新创建的汇总列。 请读者区分以下两种情况。
# group_by + summarise:分组汇总，其结果是“有几个分组就有几个观测”。
# group_by + mutate:分组修改，其结果是“原来有几个样本就有几个观测”。

# (1)summarise()函数
# 可以与很多自带或自定义的汇总函数连用，常用的汇总函数有以下几种。 
# 中心化:mean()、median()。
# 分散程度:sd()、IQR()、mad()。
# 范围:min()、max()、quantile()。
# 位置:first()、last()、nth()。 
...
# 返回值(返回向量值、甚至是数据框)的汇总函数，如 range()、quantile()等。 
qs = c(0.25, 0.5, 0.75)
df_q = df %>%
  group_by(sex) %>%
...
# dplyr 包提供了 rowwise()函数为数据框创建行化逻辑(rowwise)，使用 rowwise() 后并
# 不是真的改变了数据框，只是创建了行化逻辑的元信息，改变了数据框的操作逻辑:
rf = df %>%
  rowwise() 
...
#函数 c_across()是为按行方式(rowwise)在选定的列范围汇总数据而设计的，
#它没有提供.fns 参数，只能选择列。
rf %>%
  mutate(total = sum(c_across(where(is.numeric))))

#若只是做按行求和或均值，直接用 rowSums()或 rowMeans()函数速度更快
#(不需要“分割—汇总—合并”)，这里经过行化后提供可以做更多的按行汇总的可能。
df %>%
  mutate(total = rowSums(across(where(is.numeric))))

# 按行方式(rowwise)可以理解为一种特殊的分组，即每一行作为一组。
# 为 rowwise()函数提供 行 ID，用 summarise()函数做汇总更能体会这一点。 
# 要解除行化模式，可以使用 ungroup()函数。
df %>%
  rowwise(name) %>%
  summarise(total = sum(c_across(where(is.numeric))))

#rowwise 行化操作的缺点是速度相对更慢，更建议用 1.6.2 节讲到的 pmap()函数逐行迭代。
# rowwise 行化更让人惊喜的是:它的逐行处理的逻辑和嵌套数据框可以更好地实现批量建模，
# 在 rowwise 行化模式下，批量建模就像计算新列一样自然。
# 批量建模(参见 3.3.3 节)可以用“嵌套数据框 + purrr::map_*()”实现，
# 但这种 rowwise 技术，具有异曲同工之妙。
# 逐行迭代除了for循环通常有四种做法，具体如下: 
iris[1:4] %>% # apply
...
#2.6.2 窗口函数
# 汇总函数(如 sum()和 mean())接受 n 个输入，返回 1 个值。
# 而窗口函数是汇总函数的变体:接受 n 个输入，返回 n 个值。
# 例如，cumsum()、cummean()、rank()、lead()、lag()等函数。

# 1.排名和排序函数
# 共有6个排名函数，这里只介绍比较常用的 min_rank()函数，该函数可以实现从小到大
# 排名(ties.method="min")，若要从大到小排名需要加“−”或嵌套一个 desc()函数。 
...
#2.移位函数
#lag()函数:取前一个值，数据整体右移一位，相当于将时间轴滞后一个单位。 
#lead()函数:取后一个值，数据整体左移一位，相当于将时间轴超前一个单位。
dt = tibble(
...
# dplyr 包又提供了 cummean()、cumany()和 cumall()函数，后两者可与 filter()函数连用以选择行。
...
#2.6.3滑窗迭代
# “窗口函数”这一术语来自 SQL，意味着逐窗口浏览数据，将某函数重复应用于数据的每个 “窗口”。
...
# ...:用来传递 .f 的其他参数。
# .before 和.after:设置窗口范围当前元往前、往后几个元，可以取 Inf(即往前、往后所有元)。 
# .step:每次函数调用时，窗口往前移动的步长。 
# .complete:设置两端处是否保留不完整窗口，默认为 FALSE。 
# slider::slide_*()系列函数与 purrr::
# map_*()是类似的，只是将“逐元素迭代”换成了 “逐窗口迭代”。
# slide 滑窗迭代的作用机制如图 2.22 所示，其 逻辑是先利用窗口参数正确设计并生成滑动窗口，
# 每 个滑动窗口是一个小向量，函数.f 是作用在一个小 向量上，通过后缀控制返回结果类型，
# 返回结果通常作为 mutate 的一列。
# slide 滑窗迭代示意图 金融时间序列数据经常需要计算滑动平均值，
# 比如计算 sales 的 3 日滑动平均:
  
library(slider)
dt %>%
  mutate(avg_3 = slide_dbl(sales, mean, .before = 1, .after = 1))

#输出每个滑动窗口更便于理解该 3 日滑动平均值是如何计算的: 
slide(dt$sales, ~ .x, .before = 1, .after = 1)

# 细心的读者可能发现了:上面计算的并不是真正的 3 日滑动平均值，而是连续 3 个值的滑
# 动平均值。这是因为 slide()函数默认是以行索引来滑动，如果日期也是连续日期就没有问题。 
# 但是若日期有跳跃，则结果可能不是你想要的。
# 那么，怎么计算真正的 3 日滑动平均值呢?
# 需要改用 slide_index()函数，并提供日期 索引，其基本格式为:
# slide_index(.x, .i, .f, ...)
...
##2.6.4 整洁计算
# tidyverse 代码之所以这么整洁、优雅，并且访问列时只需要提供列名，不需要加引号， 
# 也不需要加数据框环境 df$, 这是因为它内部采用了一套整洁计算(tidy evaluation)框架。
# 如果我们也想自定义这样整洁、优雅的函数，也即在自定义函数中这样“整洁、优雅”地 传递参数，
# 就需要掌握一些整洁计算的技术，具体如下。
# 1.数据屏蔽与整洁选择 整洁计算的两种基本形式如下所示。
#   数据屏蔽:使得可以不用带数据框(环境变量) 名字，就能使用数据框内的变量(数 据变量)，
#            以便于在数据集内计算值。
#   整洁选择:即各种选择列的语法，便于使用数据集中的列。 数据屏蔽内在的机制是先冻结
#            表达式，然后注入函数，再恢复其计算。整洁计算已经为此
# 做好了两种封装，如下所示。
...
#若要传递多个整洁变量名，可以借助 across()函数传递一个整洁选择(tidy select): 
...
#若想用字符串形式传递变量名，在访问数据时需要借助.data[[var]]，这里的.data 相当
#于代替数据集的代词:
var_summary = function(data, var) {
  data %>%
...
#若传递的参数是多个列名构成的字符向量，则需要借助函数 all_of()或 any_of()，
#具体选用哪个取决于你的选择:
vars = c("mpg", "vs")
mtcars %>% select(all_of(vars))
mtcars %>% select(!all_of(vars))

#最后，再来看使用{{ }}或整洁选择同时修改列名的用法: 
...
# 要传递多个参数可以用特殊参数“...”。比如，我们还想让计算分组均值的 group_var 可以
# 是任意多个，这就需要改用“...”参数，为了更好地应付这种参数传递，我们特意将该参数
# 放在最后一个位置。
# 另外，将其他函数参数都增加前缀“.”是一个好的做法，因为可以降低 其与“...”参数的冲突
# 风险。
...
#“...”参数不需要做引用和反引用就能正确工作，但若要修改结果列名就不行了，仍需要
#借助引用和反引用，但是要改用 enques()和“!!!”。
grouped_mean = function(.data, .summary_var, ...) {
  summary_var = enquo(.summary_var)
...
# data.table 包是 data.frame 的高性能版本，不依赖其他包就能胜任各种数据操作，
# 速 度超快，让个人计算机都能轻松处理几 GB 甚至十几 GB 的数据。
# data.table 的高性能来源 于内存管理(引用语法)、并行化和大量精细优化。
# 但是，与 tidyverse 一次用一个函数做一件事， 通过管道依次连接，整洁地完成复杂事情
# 的理念截然 不同，data.table 语法高度抽象、简洁、统一,一句话概括 data.table 语法:
# 用 i 选择行，用 j 操作列，根据 by 分组。其中，j 表达式非常强大和灵活，可以选择、
# 修改、汇总和计算新列，甚至可以接受任意表 达式。需要记住的关键一点是:只要返回的 
# list 元素是等长元素或长度为 1 的元素，那么每个 list 元素将转化为结果 data.table 
# 的一列。
# data.table 高度抽象的语法无疑增加了学习成本，但它的高效性能和处理大数据能力
# 使得我们非常有必要学习它。当然，读者如果既想要 data.table 的高性能，又想要 
# tidyverse 的整洁语法，也可以借助一些衔接二者的中间包(如 dtplyr、tidyfst 等)实现。
# 为了节省篇幅，本节将只展示代码以演示 data.table 语法，请尽量忽略输出结果。
# 因为 有些复杂操作采用了同前文一样的数据，会得到同样的结果。
...
dt = data.table(
  ...
  y = c("A", "B"))
dt
# 用 as.data.table()可将数据框、列表、矩阵等转化为 data.table;
# 若只想按引用转化，则使用 setDT()函数。

# 1.引用语法
# 高效计算的编程都支持引用语法，也叫浅复制。 浅复制1只是复制列指针向量(对应数据框的列)，
# 而实际数据在内存中不做物理复制;而
# 相对的概念— 深复制，则将整个数据复制到内存中的另一位置，深复制这种冗余的复制极大 地影响性能，
# 特别是大数据的情形。
# data.table 使用“:=”运算符，做整列或部分列替换时都不做任何复制，因为“:=”运 算符是通过引用就地
# 更新 data.table 的列。
# 若想要复制数据而不想按引用处理(修改数据本身)，则使用DT2 = copy(DT1)。 
# 
# 2.键与索引
...
# .I : 整 数 向 量 seq_len(nrow(x)) ， 例 如 DT[, .I[which.max(somecol)],by=grp]。
...
# 2.7.2 数据读写
# 函数 fread()和 fwrite()是 data.table 中最强大的函数之二。它们最大的优势，仍是
# 读取大数据时速度超快，且非常稳健，分隔符、列类型、行数都可以自动检测;它们非常通用,
# 可以处理不同的文件格式(但不能直接读取 Excel 文件)，还可以接受 URL 甚至是操作系统指令。
#1.读入数据 
fread("DT.csv")
fread("DT.txt", sep = "\t")
# 选择部分行列读取
...
# 1.左连接
# 外连接至少保留一个数据表中的所有观测，分为左连接、右连接、全连接，其中最常用的是
# 左连接:保留 x 的所有行，合并匹配的 y 中的列。
# 若表x与y中匹配列的列名不同，可以用by.x = "c1", by.y = "c2"，若有多个匹配列，嵌套
# 使用 c()即可。
# 上面代码提供了左连接的三种不同实现，为了易记性和可读性，更建议用第三种 merge()函数。 
# 注意，只要加载了 data.table 包，程序将自动使用更快速的 data.table::merge()函数，
# 而不是base R中的merge()函数，尽管二者语法相同。 
# 2.右连接
# 保留 y 中的所有行，合并匹配的 x 中的列: 
...
funion(x, y)
fsetequal(x, y)
y[x, on = "v1"] # 注意是以x为左表 y[x] 
# 若v1是键 merge(x, y, all.x = TRUE, by = "v1")

# 2.7.4 数据重塑
# 1.宽表变长表
# 宽表的特点是表比较宽，本来该是“值”的，却出现在“变量(名)”中。这就需要给它变 到“值”中，
# 新起个列名存为一列，这就是所谓的宽表变长表。
# 每一行只有 1 个观测的情形
# 参数 measure 是用整数向量指定要变形的列，也可以使用正则表达式 patterns("年$"), 
# 也可以改用参数id指定不变形的列;若需要忽略缺失值，可以设置参数na.rm = TRUE。
# 基于 tidyr::pivot_longer()的实现如下: 
DT %>%
  pivot_longer(-地区, names_to = "年份", values_to = "GDP")
# 两种语法基本相同，都是指定要变形的列，为存放变形列的列名中的“值”指定新列名，
# 为存放变形列中的“值”指定新列名。 
# 每一行有多个观测的情形
...
# 2.长表变宽表
# 长表的特点是表比较长。有时候需要将分类变量的若干水平值变成变量(列名)，这就是长表
# 变宽表，它与宽表变长表正好相反(二者互逆)。
# 只有 1 个列名列和 1 个值列的情形
load("data/animals.rda")
DT = as.data.table(animals) # 农场动物数据 DT %>%
dcast(Year ~ Type, value = "Heads", fill = 0)
# 基于 tidyr::pivot_wider()的实现如下:
DT %>%
  pivot_wider(names_from = Type, values_from = Heads, values_fill = 0)
# dcast()函数的第 1 个参数是公式形式，~ 左边是不变的列，右边是“变量名”来源列，
# 参数 value 指定“值”的来源列。
# 有多个列名列和多个值列的情形
us_rent_income %>%
  as.data.table() %>%
...
#tidyverse 是用 mutate()函数修改列，此时并不会修改原数据框，必须赋值才能看到结果变化结果;
#data.table修改列是用列赋值符号“:=”(不执行复制)，可以直接对原数据框修改。
#修改或增加一列
#注意，代码v3 = v2 + 1中的v2是原始的v2列，而不是前面新计算的v2列;
#若想使用新计算的列， 可以用以下语句:
dt[, c("v2", "v3") := .(temp <- log(v1), v3 = temp + 1)]

#增加多列
#同时修改多列
#tidyverse 是借助 across()函数或_all、_if、_at 后缀选择并同时操作多列;
#而data.table 选择并操作多列是借助 lapply()函数以及特殊符号。
#.SD:每个分组数据(除了 by 或 keyby 的列)。
...
# 注意，上述同时修改多列的代码，都是只保留新列，若要保留所有列，需要准备新列名 cols, 
# 再在 j 表 达式中使用(cols):= ...
#删除列
dt[, v1 := NULL]
dt[, c("v2","v3") := NULL]
...
#2.7.6 分组操作
#用 by 表达式指定分组。
# data.table 是根据 by 或 keyby 分组，区别是，keyby 会排序结果并创建键，使得我们
# 可以更快地访问子集。
# 未分组数据框相当于把整个数据框作为 1 组，数据操作是在整个数据框上进行，若汇总则
# 得到的是 1 个结果。
# 分组数据框相当于把整个数据框分成了 m 个数据框，数据操作是分别在每个数据框上进行，
# 若汇总则得到的是 m 个结果。
# 使用带NA值的考试成绩数据
...
#data.table 提供快速处理日期时间的 IDateTime 类，更多信息可查阅帮助文档。 
#对某些列做汇总
DT[, lapply(.SD, mean), .SDcols = patterns("h"), by = .(class, sex)]
# 或用by = c("class", "sex")  对所有列做汇总
DT[, name := NULL][, lapply(.SD, mean, na.rm = TRUE),by = .(class, sex)]
#对满足条件的列做汇总
DT[, lapply(.SD, mean, na.rm = TRUE), by = class,.SDcols = is.numeric]
#分组计数
DT = na.omit(DT)
DT[, .N, by = .(class, cut(math, c(0, 60, 100)))] %>%print(topn = 2)
#上述分组计数会忽略频数为 0 的分组，若要显示频数为 0 的分组可以用以下方法: 
DT[, Bin := cut(math, c(0, 60, 100))]
#其中，函数 CJ()相当于 expand_grid()，可以生成所有两两组合(笛卡儿积)。 
#分组选择行
#data.table 也提供了辅助函数:first()、last()、uniqueN()，比如提取每组的
#first/nth 观测，可以使用以下方式:
